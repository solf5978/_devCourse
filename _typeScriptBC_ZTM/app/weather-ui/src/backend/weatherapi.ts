...
export class CurrentWeather {
  ...
  constructor(apiResponse: CurrentWeatherApiResponse) {
    this.temperature = {
      ...
      unit: apiResponse.hourly_unit.temperature_2m,
    };
  ...
  lowTemp(): number {
    return this.hourlyTemp.reduce((prev, curr) => Math.min(prev, curr));
  }

  highTemp(): number {
    return this.hourlyTemp.reduce((prev, curr) => Math.max(prev, curr));
  }
}
