...
For this challenge, you are going to use all the assignment operators to change 
the value of a `number` variable, but the final result will be the same as the 
starting number. Feel free to print `number` to the console after each operation to see its value.
"""
...
# Use the exponentiation assignment operator to raise `number` to the third 
# power
number **= 3

# Use the modulus assignment operator to divide `number` by 727 and get the 
# remainder
number %= 727
